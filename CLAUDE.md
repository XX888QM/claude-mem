...
- Non-quota generator failures that leave buffered work behind preserve it for the normal three-minute idle window; the next ingest or `POST /api/sessions/init` (with matching `platformSource`) restarts the generator, while an abandoned session finalizes when that window expires. Quota pauses remain preserved for a later ingest. A clean exit with an empty buffer finalizes immediately. The queue is in-memory only — restarting the worker drops it.
