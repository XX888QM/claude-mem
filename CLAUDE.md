...
- Viewer labels are the host app, not the model. Cursor's Claude Code compatibility layer can replay the same conversation as `hook claude-code`; those ghosts are skipped (`cursor_compat_duplicate`) so one conversation stays a single CURSOR card. Real Claude Code sessions stay CLAUDE. SessionStart inject is cross-source and does not pass `platformSource`.
