...
/** Raised when a caller's bounded cleanup deadline expires before a signal is sent. */
export class ProcessTreeKillAbortedError extends Error {
  readonly pid: number;
  constructor(pid: number) {
    super(`tree-kill aborted for PID ${pid}`);
    this.name = 'ProcessTreeKillAbortedError';
    this.pid = pid;
  }
}
...
export interface KillProcessTreeOptions {
  ...
  /** Stop before any later signal when a bounded caller runs out of time. */
  signal?: AbortSignal;
}
...
export async function killProcessTree(
  ...
  const throwIfAborted = (): void => {
    if (options.signal?.aborted) throw new ProcessTreeKillAbortedError(pid);
  };
  throwIfAborted();
  ...
  try {
    ...
    if (!immediate) {
      await new Promise<void>((resolve, reject) => {
        ...
        options.signal?.addEventListener('abort', () => {
          ...
          reject(new ProcessTreeKillAbortedError(pid));
        }, { once: true });
      });
    }
    ...
    if (error instanceof ProcessTreeKillAbortedError) throw error;
