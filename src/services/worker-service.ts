...
import { GrokProvider, isGrokSelected } from './worker/GrokProvider.js';
...
export class WorkerService implements WorkerRef {
  ...
  constructor() {
    ...
    this.grokAgent = new GrokProvider(this.dbManager, this.sessionManager);
    ...
    // Always mount the remote read-only guard. An empty token denies every
    // non-loopback request; official only mounts when the token is set.
    ...
    this.server = new Server({
      ...
      getAiStatus: () => {
        let provider = 'claude';
        if (isGrokSelected()) provider = 'grok';
        else if (isCodexSelected()) provider = 'codex';
        else if (isOpenRouterSelected() && isOpenRouterAvailable()) provider = 'openrouter';
        else if (isGeminiSelected() && isGeminiAvailable()) provider = 'gemini';
        return {
          provider,
          authMethod: provider === 'grok'
            ? 'Grok CLI login'
            : provider === 'codex'
              ? 'Codex CLI login'
      ...
      remoteReadOnly: { getToken: () => this.tvToken },
    });
  ...
  private registerRoutes(): void {
    ...
    const sessionRoutes = new SessionRoutes(this.sessionManager, this.dbManager, this.sdkAgent, this.geminiAgent, this.openRouterAgent, this.codexAgent, this.grokAgent, this.sessionEventBroadcaster, this, this.completionHandler);
