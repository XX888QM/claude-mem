...
| `0.0.0.0` | **empty** | This local fork denies every remote request. Direct loopback access remains available. |
...
Booting with a non-loopback host and no token logs a `SECURITY` warning; the worker still binds but the always-mounted guard denies remote requests.
