...
describe.if(isPosix)('killProcessTree signal modes', () => {
  ...
  it('does not signal a tree when cancellation was requested before cleanup', async () => {
    const { rootPid } = spawnTwoLevelTree();
    await settle();
    const controller = new AbortController();
    controller.abort();
    const options = { signalMode: 'immediate' as const, signal: controller.signal };

    try {
      await expect(killProcessTree(rootPid, options)).rejects.toThrow('aborted');
