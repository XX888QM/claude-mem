...
describe('ChromaMcpManager singleton enforcement (#2313)', () => {
  ...
  it('does not reconnect an active mutation after shutdown starts', async () => {
    ...
    // Fork drain semantics: stop waits for in-flight work, so surface the
    // transport failure while stop is draining (same pattern as direct-call test).
    const stopPromise = mgr.stop();
    rejectMutation?.(new Error('Connection closed'));

    await expect(pendingMutation).rejects.toThrow(/cancelled during shutdown|Connection closed/);
    await stopPromise;
    expect(transportInstances.length).toBe(1);
  });
